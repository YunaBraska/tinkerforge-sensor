...
import static berlin.yuna.tinkerforgesensor.model.type.ValueType.HEADING;
 ...
 * <li>{@link ValueType#HEADING} [x / 10 = °]</li>
 * </ul>
...
public class Compass extends Sensor<BrickletCompass> {
    ...
    protected Sensor<BrickletCompass> initListener() {
        device.addHeadingListener(heading -> sendEvent(HEADING, heading));
    ...
    public Sensor<BrickletCompass> refreshPeriod(final int milliseconds) {
        try {
            ...
            sendEvent(HEADING, device.getHeading());
        } catch (TinkerforgeException ignored) {
            sendEvent(DEVICE_TIMEOUT, 404L);
        }
        return this;
