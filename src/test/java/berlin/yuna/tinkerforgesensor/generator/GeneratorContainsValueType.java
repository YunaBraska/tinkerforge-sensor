...
public class GeneratorContainsValueType {
    ...
    public static JavaFile generate() {
        ...
        //GENERATE METHODS
        resultClass.addMethods(stream(ValueType.values()).map(GeneratorContainsValueType::methodInnerIsTypeOf).collect(toSet()));
    ...
    private static MethodSpec methodInnerIsTypeOf(final ValueType valueType) {
        return MethodSpec.methodBuilder(toHumanReadable(valueType, false))
                ...
                .addStatement("return $N.contains($L)", FIELD_TYPE_NAME, valueType)
                .build();
    }
