...
public class GeneratorSensors {
    ...
    public static JavaFile generate(final List<Class<? extends Sensor>> sensors) {
        ...
        while (!sensorList.isEmpty()) {
            ...
            final String methodName = firstLetterLow(getBasicSensorName(sensor.getSimpleName()));

            if (!"default".equals(methodName)) {
                resultClass.addMethod(method_getSensorXY(methodName));
                resultClass.addMethod(method_getSensorXYNumber(methodName, sensorVersions));
            }
    ...
    private static MethodSpec method_getDefaultSensor(final ParameterizedTypeName type_ListOfSensor, final ClassName type_ClassOfGenerics) {
        ...
        return MethodSpec.methodBuilder("getDefaultSensor")
                ...
                .addCode(
                        "try {\n  return new $T($N);\n} catch ($T e) {\n  throw new $T(\"Default device should not run into an exception\", e);\n}\n",
    ...
    private static MethodSpec method_getSensor(final ParameterizedTypeName type_ListOfSensor, final ClassName type_ClassOfGenerics) {
        ...
        return MethodSpec.methodBuilder("get" + Sensor.class.getSimpleName())
                ...
                .addStatement("return stream().filter(sensor -> sensor.isClassType($N)).sorted($N($T::port)).collect($N)", parameterName, "java.util.Comparator.comparingInt", Sensor.class, "java.util.stream.Collectors.toList()")
                .build();
    }
...
//    }
    ...
    private static MethodSpec method_getSensorXYNumber(final String methodName, final List<Class<? extends Sensor>> sensorVersions) {
