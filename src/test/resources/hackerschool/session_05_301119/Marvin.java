...
public class Marvin extends Helper {
    ...
    static void onSensorEvent(final SensorEvent event) {
        ...
        if(event.getValueType().isButtonTouch() && event.getValues().get(9).intValue() == 1){
                move(10000, -10000);
            }
            if(event.getValueType().isButtonTouch() && event.getValues().get(9).intValue() == 0 ){
                move(0, -0);
        }
        if(event.getValueType().isButtonTouch() && event.getValues().get(11).intValue() == 1){
            move(5000, 5000);
        }
        if(event.getValueType().isButtonTouch() && event.getValues().get(11).intValue() == 0){
            move(0, 0);
        }
        ...
        if (event.sensor().compare().is(stack.sensors().rotary()) && event.getValueType().isButtonPressed()) {
            sirene = !sirene;
        }
    ...
    public static void main(final String[] args) throws Exception {
        ...
        stack.consumers.add(Marvin::onSensorEvent);
